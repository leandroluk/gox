...
In `validate`, you create a lightweight schema and call `.Validate()`.

| Feature    | govalidator                 | validate                                  |
| :--------- | :-------------------------- | :---------------------------------------- |
| **Email**  | `govalidator.IsEmail(str)`  | `validate.Text().Email().Validate(str)`   |
| **URL**    | `govalidator.IsURL(str)`    | `validate.Text().URL().Validate(str)`     |
| **UUID**   | `govalidator.IsUUID(str)`   | `validate.Text().UUID().Validate(str)`    |
| **Int**    | `govalidator.IsInt(str)`    | `validate.Text().Numeric().Validate(str)` |
| **Base64** | `govalidator.IsBase64(str)` | `validate.Text().Base64().Validate(str)`  |
| **JSON**   | `govalidator.IsJSON(str)`   | (No direct equivalent, unmarshal instead) |

> **Note**: `validate` returns `(value, error)`. If you just want a boolean, check `err == nil`.
...
| govalidator Tag       | validate Equivalent                  |
| :-------------------- | :----------------------------------- |
| `required`            | `.Required()`                        |
| `email`               | `.Email()`                           |
| `url`                 | `.URL()`                             |
